"""
IFNet - Intermediate Flow Network for RIFE
Main neural network for optical flow estimation
"""
...
class IFNet(nn.Module):
    ...
    Multi-scale flow estimation network for frame interpolation.
    """
    ...
    def forward(self, x, scale=[4, 2, 1], timestep=0.5):
        img0 = x[:, :3]
        img1 = x[:, 3:6]
        gt = x[:, 6:]  # Ground truth (empty during inference)
        ...
        # Teacher network (only used during training with GT)
        if gt.shape[1] == 3:
